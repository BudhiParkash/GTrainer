...
public class All_List_Trainer extends AppCompatActivity {
   ...
   private List<User>  alltrainerList = new ArrayList<>();
        ...
        call.enqueue(new Callback<List<User>>() {
            ...
            public void onResponse(Call<List<User>> call, Response<List<User>> response) {
                if(response.code() == 200){
                    alltrainerList = response.body();
                    assert alltrainerList != null;
                    Toast.makeText(All_List_Trainer.this, "Success all trainer", Toast.LENGTH_SHORT).show();
                    if(alltrainerList.size() == 0){
                    ...
                    Toast.makeText(All_List_Trainer.this, ""+response.code(), Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void onFailure(Call<List<User>> call, Throwable t) {
                Toast.makeText(All_List_Trainer.this, ""+t.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
